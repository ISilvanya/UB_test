...
Процедура ФормаЗадачиПриСозданииНаСервере(ФормаЗадачи, ЗадачаОбъект, 
	...
	Если УБ_БизнесПроцессыИЗадачиВызовСервера.ЭтоВедущаяЗадача(ЗадачаОбъект.Ссылка) Тогда
		Если ЭлементГруппаСостояние <> Неопределено Тогда
			ЭлементГруппаСостояние.Видимость = Истина;
		КонецЕсли;
		Родитель = ?(ЭлементГруппаСостояние <> Неопределено, ЭлементГруппаСостояние, ФормаЗадачи);
...
Процедура УстановитьПометкуУдаленияЗадач(БизнесПроцессСсылка, ПометкаУдаления) Экспорт
	...
	Попытка
		...
		ЭлементБлокировки = Блокировка.Добавить("Задача.УБ_ЗадачаИсполнителя");
		...
		Запрос = Новый Запрос("ВЫБРАТЬ
			...
			|ИЗ
			|	Задача.УБ_ЗадачаИсполнителя КАК Задачи
			|ГДЕ
...
Функция ДатаЗавершенияБизнесПроцесса(БизнесПроцессСсылка) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	МАКСИМУМ(УБ_ЗадачаИсполнителя.ДатаИсполнения) КАК МаксДатаИсполнения
		|ИЗ
		|	Задача.УБ_ЗадачаИсполнителя КАК УБ_ЗадачаИсполнителя
		|ГДЕ
		|	УБ_ЗадачаИсполнителя.БизнесПроцесс = &БизнесПроцесс
		|	И УБ_ЗадачаИсполнителя.Выполнена = ИСТИНА";
	Запрос.УстановитьПараметр("БизнесПроцесс", БизнесПроцессСсылка);
...
Процедура ПроверитьПраваНаИзменениеСостоянияБизнесПроцесса(БизнесПроцессОбъект) Экспорт
	...
	Если Не ЗначениеЗаполнено(БизнесПроцессОбъект.Состояние) Тогда 
		БизнесПроцессОбъект.Состояние = Перечисления.УБ_СостоянияБизнесПроцессов.Активен;
	КонецЕсли;
	
	Если БизнесПроцессОбъект.ЭтоНовый() Тогда
		ПредыдущееСостояние = Перечисления.УБ_СостоянияБизнесПроцессов.Активен;
	Иначе
	...
	Если ПредыдущееСостояние <> БизнесПроцессОбъект.Состояние Тогда
		...
		Если ПредыдущееСостояние = Перечисления.УБ_СостоянияБизнесПроцессов.Активен Тогда
		...
		ИначеЕсли ПредыдущееСостояние = Перечисления.УБ_СостоянияБизнесПроцессов.Остановлен Тогда
...
Процедура ЗаполнитьГлавнуюЗадачу(БизнесПроцессОбъект, ДанныеЗаполнения) Экспорт
	...
	УБ_БизнесПроцессыИЗадачиПереопределяемый.ПриЗаполненииГлавнойЗадачиБизнесПроцесса(БизнесПроцессОбъект, ДанныеЗаполнения, СтандартнаяОбработка);
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ЗадачаСсылка.УБ_ЗадачаИсполнителя") Тогда
...
Функция ГруппаИсполнителейЗадач(РольИсполнителя, ОсновнойОбъектАдресации, ДополнительныйОбъектАдресации) Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	УБ_ГруппыИсполнителейЗадач.Ссылка КАК Ссылка
			|ИЗ
			|	Справочник.УБ_ГруппыИсполнителейЗадач КАК УБ_ГруппыИсполнителейЗадач
			|ГДЕ
			|	УБ_ГруппыИсполнителейЗадач.РольИсполнителя = &РольИсполнителя
			|	И УБ_ГруппыИсполнителейЗадач.ОсновнойОбъектАдресации = &ОсновнойОбъектАдресации
			|	И УБ_ГруппыИсполнителейЗадач.ДополнительныйОбъектАдресации = &ДополнительныйОбъектАдресации");
		...
		Иначе
			...
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("Справочник.УБ_ГруппыИсполнителейЗадач");
...
Процедура ДобавитьПроцессДляОтложенногоСтарта(Процесс, ДатаСтарта) Экспорт
	...
	НаборЗаписей = РегистрыСведений.УБ_ПроцессыДляЗапуска.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Владелец.Установить(Процесс);
	
	Запись = НаборЗаписей.Добавить();
	Запись.Владелец = Процесс;
	Запись.ДатаОтложенногоСтарта = ДатаСтарта;
	Запись.Состояние = Перечисления.УБ_СостоянияПроцессовДляЗапуска.ГотовКСтарту;
...
Процедура ОтключитьОтложенныйСтартПроцесса(Процесс) Экспорт
	...
	Если НастройкаСтарта.Состояние = ПредопределенноеЗначение("Перечисление.УБ_СостоянияПроцессовДляЗапуска.ГотовКСтарту") Тогда
		НаборЗаписей = РегистрыСведений.УБ_ПроцессыДляЗапуска.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Владелец.Установить(Процесс);
		НаборЗаписей.Записать();
	КонецЕсли;
...
Процедура СтартоватьОтложенныйПроцесс(БизнесПроцесс) Экспорт
	...
	Попытка
		...
		РегистрыСведений.УБ_ПроцессыДляЗапуска.ЗарегистрироватьСтартПроцесса(БизнесПроцесс);
	...
	Исключение
		...
		Описание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстОшибки,
			ОписаниеОшибки);
		РегистрыСведений.УБ_ПроцессыДляЗапуска.ЗарегистрироватьОтменуСтарта(БизнесПроцесс, Описание);
...
Функция ПараметрыОтложенногоПроцесса(Процесс) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	УБ_ПроцессыДляЗапуска.Владелец,
		|	УБ_ПроцессыДляЗапуска.ДатаОтложенногоСтарта,
		|	УБ_ПроцессыДляЗапуска.Состояние,
		|	УБ_ПроцессыДляЗапуска.ПричинаОтменыСтарта
		|ИЗ
		|	РегистрСведений.УБ_ПроцессыДляЗапуска КАК УБ_ПроцессыДляЗапуска
		|ГДЕ
		|	УБ_ПроцессыДляЗапуска.Владелец = &БизнесПроцесс";
...
Функция ДатаОтложенногоСтартаПроцесса(БизнесПроцесс) Экспорт
	...
	Если Настройка.Состояние = ПредопределенноеЗначение("Перечисление.УБ_СостоянияПроцессовДляЗапуска.ГотовКСтарту") Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "УБ_БизнесПроцессыИЗадачиСервер.ЗаполнитьОтветственногоЗаКонтрольИсполнения";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.Версия              = "2.3.3.70";
	Обработчик.Процедура           = "УБ_БизнесПроцессыИЗадачиСервер.ОбновитьИспользованиеРегламентныхЗаданий";
	...
	Обработчик.НачальноеЗаполнение = Истина;
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Процедура = "УБ_БизнесПроцессыИЗадачиСервер.ЗаполнитьНаименованиеПредопределенногоЭлементаВсеОбъектыАдресации";
...
Процедура ПриОпределенииГруппИсполнителей(МенеджерВременныхТаблиц, СодержаниеПараметра, ЗначениеПараметра, НетГруппИсполнителей) Экспорт
	...
	Если СодержаниеПараметра = "ГруппыИсполнителей" Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	УБ_ИсполнителиЗадач.ГруппаИсполнителейЗадач КАК ГруппаИсполнителей,
		|	УБ_ИсполнителиЗадач.Исполнитель КАК Пользователь
		|ПОМЕСТИТЬ ТаблицаГруппИсполнителей
		|ИЗ
		|	РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач
		|ГДЕ
		|	УБ_ИсполнителиЗадач.ГруппаИсполнителейЗадач В(&ГруппыИсполнителей)";
	...
	ИначеЕсли СодержаниеПараметра = "Исполнители" Тогда
		...
		|	РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач
		|ГДЕ
		|	УБ_ИсполнителиЗадач.Исполнитель В(&Исполнители)";
...
Процедура ПриОпределенииСправочниковДляЗагрузкиДанных(ЗагружаемыеСправочники) Экспорт
	...
	СтрокаТаблицы = ЗагружаемыеСправочники.Найти(Метаданные.Справочники.УБ_ГруппыИсполнителейЗадач.ПолноеИмя(), "ПолноеИмя");
...
Процедура ПриОпределенииОбъектовСРедактируемымиРеквизитами(Объекты) Экспорт
	Объекты.Вставить(Метаданные.БизнесПроцессы.УБ_Задание.ПолноеИмя(), "РеквизитыРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.Задачи.УБ_ЗадачаИсполнителя.ПолноеИмя(), "РеквизитыРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.ПланыВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ПолноеИмя(), "РеквизитыНеРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.Справочники.УБ_ГруппыИсполнителейЗадач.ПолноеИмя(), "РеквизитыНеРедактируемыеВГрупповойОбработке");
	Объекты.Вставить(Метаданные.Справочники.УБ_РолиИсполнителей.ПолноеИмя(), "РеквизитыНеРедактируемыеВГрупповойОбработке");
КонецПроцедуры
...
Процедура ПриДобавленииИсключенийПоискаСсылок(ИсключенияПоискаСсылок) Экспорт
	...
	ИсключенияПоискаСсылок.Добавить(Метаданные.РегистрыСведений.УБ_ИсполнителиЗадач.ПолноеИмя());
...
Процедура ПриЗаполненииСпискаРеквизитовИсточникаСДатамиДляНапоминания(Источник, РеквизитыСДатами) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ЗадачаСсылка.УБ_ЗадачаИсполнителя") Тогда
...
Процедура ПриЗаполненииЗависимостейПравДоступа(ЗависимостиПрав) Экспорт
	...
	Строка.ПодчиненнаяТаблица = "Задача.УБ_ЗадачаИсполнителя";
	Строка.ВедущаяТаблица     = "БизнесПроцесс.УБ_Задание";
...
Процедура ПриЗаполненииВидовОграниченийПравОбъектовМетаданных(Описание) Экспорт
	...
	Описание = Описание 
		+ "
		|БизнесПроцесс.УБ_Задание.Чтение.Пользователи
		|БизнесПроцесс.УБ_Задание.Изменение.Пользователи
		|Задача.УБ_ЗадачаИсполнителя.Чтение.Объект.БизнесПроцесс.УБ_Задание
		|Задача.УБ_ЗадачаИсполнителя.Чтение.Пользователи
		|Задача.УБ_ЗадачаИсполнителя.Изменение.Пользователи
		|РегистрСведений.ДанныеБизнесПроцессов.Чтение.Объект.БизнесПроцесс.УБ_Задание
		|";
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	Если ВидДоступа <> Неопределено Тогда
		...
		МодульУправлениеДоступом.ДобавитьДополнительныеТипыВидаДоступа(ВидДоступа,
			Тип("СправочникСсылка.УБ_ГруппыИсполнителейЗадач"));
	КонецЕсли;
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.РегистрыСведений.УБ_ИсполнителиЗадач, Истина);
	Списки.Вставить(Метаданные.БизнесПроцессы.УБ_Задание, Истина);
	Списки.Вставить(Метаданные.Задачи.УБ_ЗадачаИсполнителя, Истина);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Редактирование", Метаданные.Задачи.УБ_ЗадачаИсполнителя)
		Или МодульТекущиеДелаСервер.ДелоОтключено("ЗадачиИсполнителя") Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ПолучитьФункциональнуюОпцию("УБ_ИспользоватьБизнесПроцессыИЗадачи") Тогда
		Возврат;
	КонецЕсли;
	...
	Разделы = МодульТекущиеДелаСервер.РазделыДляОбъекта(Метаданные.Задачи.УБ_ЗадачаИсполнителя.ПолноеИмя());
	
	Если Пользователи.ЭтоСеансВнешнегоПользователя()
		И Разделы.Количество() = 0 Тогда
		Разделы.Добавить(Метаданные.Задачи.УБ_ЗадачаИсполнителя);
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Форма          = "Задача.УБ_ЗадачаИсполнителя.Форма.МоиЗадачи";
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	...
	СоответствиеИменПсевдонимам.Вставить("УБ_БизнесПроцессыИЗадачиСобытия.СтартОтложенныхПроцессов");
...
Процедура ПриОпределенииОбъектовСНачальнымЗаполнением(Объекты) Экспорт
	...
	Объекты.Добавить(Метаданные.Справочники.УБ_РолиИсполнителей);
	Объекты.Добавить(Метаданные.ПланыВидовХарактеристик.УБ_ОбъектыАдресацииЗадач);
...
Процедура ПриОпределенииОбъектовСКомандамиСозданияНаОсновании(Объекты) Экспорт
	...
	Объекты.Добавить(Метаданные.БизнесПроцессы.УБ_Задание);
	Объекты.Добавить(Метаданные.Задачи.УБ_ЗадачаИсполнителя);
...
Функция НайтиИсполнителейПоРолям(Знач Задача, Знач БазовыйТекстЗапроса)
	...
	Если ЗначениеЗаполнено(ДОА) Тогда
		ТекстЗапроса = БазовыйТекстЗапроса + " И УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = &ООА
		                                     |И УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = &ДОА";
	ИначеЕсли ЗначениеЗаполнено(ООА) Тогда
		ТекстЗапроса = БазовыйТекстЗапроса 
			+ " И УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = &ООА
		    |И (УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = ЗНАЧЕНИЕ(ПланВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ПустаяСсылка)
			|   ИЛИ УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = Неопределено)";
	Иначе
		ТекстЗапроса = БазовыйТекстЗапроса 
			+ " И (УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = ЗНАЧЕНИЕ(ПланВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ПустаяСсылка)
			|    ИЛИ УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = Неопределено)
		    |И (УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = ЗНАЧЕНИЕ(ПланВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ПустаяСсылка)
			|   ИЛИ УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = Неопределено)";
	КонецЕсли;
	...
	Если ВыгрузкаИсполнителей.Количество() = 0 Тогда
		ТекстЗапроса = БазовыйТекстЗапроса + " И (УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = ЗНАЧЕНИЕ(ПланВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ПустаяСсылка)
			|    ИЛИ УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = Неопределено)
			|И (УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = ЗНАЧЕНИЕ(ПланВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ПустаяСсылка)
			|   ИЛИ УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = Неопределено)";
...
Функция НайтиОтветственныхЗаНазначениеРолей(Знач Задача)
	...
	БазовыйТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ УБ_ИсполнителиЗадач.Исполнитель
	                      |ИЗ
	                      |	РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач, Справочник.УБ_РолиИсполнителей КАК УБ_РолиИсполнителей
	                      |ГДЕ
	                      |	УБ_ИсполнителиЗадач.РольИсполнителя = УБ_РолиИсполнителей.Ссылка
	                      |И
	                      |	УБ_РолиИсполнителей.Ссылка = ЗНАЧЕНИЕ(Справочник.УБ_РолиИсполнителей.ОтветственныйЗаКонтрольИсполнения)";
...
Функция ВыбратьИсполнителейЗадач(Знач Задача)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ
				  |	УБ_ИсполнителиЗадач.Исполнитель КАК Исполнитель
				  |ИЗ
				  |	РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач
				  |ГДЕ
				  |	УБ_ИсполнителиЗадач.РольИсполнителя = &РольИсполнителя
				  |	И УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = &ОсновнойОбъектАдресации
				  |	И УБ_ИсполнителиЗадач.ДополнительныйОбъектАдресации = &ДополнительныйОбъектАдресации";
...
Функция ВыбратьПросроченныеЗадачи()
	...
	Пока Индекс > 0 Цикл
		...
		Если НЕ ЗначениеЗаполнено(ПросроченнаяЗадача.Исполнитель) И УБ_БизнесПроцессыИЗадачиВызовСервера.ЭтоВедущаяЗадача(ПросроченнаяЗадача.Ссылка) Тогда
...
Функция СписокПросроченныхЗадач()
	...
	ТекстЗапроса = 
		...
		|	УБ_ЗадачаИсполнителя.СрокИсполнения КАК СрокИсполнения,
		|	УБ_ЗадачаИсполнителя.Исполнитель КАК Исполнитель,
		|	УБ_ЗадачаИсполнителя.РольИсполнителя КАК РольИсполнителя,
		|	УБ_ЗадачаИсполнителя.ОсновнойОбъектАдресации КАК ОсновнойОбъектАдресации,
		|	УБ_ЗадачаИсполнителя.ДополнительныйОбъектАдресации КАК ДополнительныйОбъектАдресации,
		|	УБ_ЗадачаИсполнителя.Автор КАК Автор,
		|	УБ_ЗадачаИсполнителя.Описание КАК Описание
		|ИЗ
		|	Задача.УБ_ЗадачаИсполнителя КАК УБ_ЗадачаИсполнителя
		|ГДЕ
		|	УБ_ЗадачаИсполнителя.ПометкаУдаления = ЛОЖЬ
		|	И УБ_ЗадачаИсполнителя.Выполнена = ЛОЖЬ
		|	И УБ_ЗадачаИсполнителя.СрокИсполнения <= &Дата
		|	И УБ_ЗадачаИсполнителя.СостояниеБизнесПроцесса <> ЗНАЧЕНИЕ(ПеречисленУБ_СостоянияБизнесПроцессовсов.Остановлен)";
...
Процедура СоздатьЗадачуПоНастройкеРолей(ЗадачаСсылка, Ответственные)
	...
	Для Каждого Ответственный Из Ответственные Цикл
		ЗадачаОбъект = Задачи.УБ_ЗадачаИсполнителя.СоздатьЗадачу();
		ЗадачаОбъект.Дата = ТекущаяДатаСеанса();
		ЗадачаОбъект.Важность = Перечисления.УБ_ВариантыВажностиЗадачи.Высокая;
...
Функция ВыбратьНовыеЗадачиПоИсполнителям(Знач ДатаВремяОт, Знач ДатаВремяПо)
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	УБ_ЗадачаИсполнителя.Ссылка КАК Ссылка,
		|	УБ_ЗадачаИсполнителя.Номер КАК Номер,
		|	УБ_ЗадачаИсполнителя.Дата КАК Дата,
		|	УБ_ЗадачаИсполнителя.Наименование КАК Наименование,
		|	УБ_ЗадачаИсполнителя.СрокИсполнения КАК СрокИсполнения,
		|	УБ_ЗадачаИсполнителя.Автор КАК Автор,
		|	УБ_ЗадачаИсполнителя.Описание КАК Описание,
		|	ВЫБОР
		|		КОГДА УБ_ЗадачаИсполнителя.Исполнитель <> НЕОПРЕДЕЛЕНО
		|			ТОГДА УБ_ЗадачаИсполнителя.Исполнитель
		|		ИНАЧЕ УБ_ИсполнителиЗадач.Исполнитель
		...
		|ИЗ
		|	Задача.УБ_ЗадачаИсполнителя КАК УБ_ЗадачаИсполнителя
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач
		|		ПО УБ_ЗадачаИсполнителя.РольИсполнителяУБ_ИсполнителиЗадачдач.РольИсполнителя
		|			И УБ_ЗадачаИсполнителя.ОсновнойОбъектАдресацииУБ_ИсполнителиЗадачдач.ОсновнойОбъектАдресации
		|			И УБ_ЗадачаИсполнителя.ДополнительныйОбъектАдресацииУБ_ИсполнителиЗадачдач.ДополнительныйОбъектАдресации
		|ГДЕ
		|	УБ_ЗадачаИсполнителя.Выполнена = ЛОЖЬ
		|	И УБ_ЗадачаИсполнителя.Дата МЕЖДУ &ДатаВремяОт И &ДатаВремяПо
		|	И УБ_ЗадачаИсполнителя.ПометкаУдаления = ЛОЖЬ
		|	И (УБ_ЗадачаИсполнителя.Исполнитель <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
		|			ИЛИ УБ_ИсполнителиЗадач.Исполнитель ЕСТЬ НЕ NULL 
		|		И УБ_ИсполнителиЗадач.Исполнитель <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка))
		|
		|УПОРЯДОЧИТЬ ПО
		|	Исполнитель,
		|	СрокИсполнения УБЫВ
...
Функция ВыбратьРолиСКоличествомИсполнителей(ОсновнойОбъектАдресации) Экспорт
	Если ОсновнойОбъектАдресации <> Неопределено Тогда
		ТекстЗапроса = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	УБ_РолиИсполнителей.Ссылка КАК РольСсылка,
			|	УБ_РолиИсполнителей.Наименование КАК Роль,
			|	УБ_РолиИсполнителей.ВнешняяРоль КАК ВнешняяРоль,
			|	УБ_РолиИсполнителей.ТипыОсновногоОбъектаАдресации КАК ТипыОсновногоОбъектаАдресации,
			|	СУММА(ВЫБОР
			|			КОГДА УБ_ИсполнителиЗадач.РольИсполнителя <> ЗНАЧЕНИЕ(Справочник.УБ_РолиИсполнителей.ПустаяСсылка) 
			|				И УБ_ИсполнителиЗадач.РольИсполнителя ЕСТЬ НЕ NULL 
			|				И УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = &ОсновнойОбъектАдресации
			|				ТОГДА 1
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК Исполнители
			|ИЗ
			|	Справочник.УБ_РолиИсполнителей КАК УБ_РолиИсполнителей
			|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач
			|		ПО (УБ_ИсполнителиЗадач.РольИсполнителя = УБ_РолиИсполнителей.Ссылка)
			|ГДЕ
			|	УБ_РолиИсполнителей.ПометкаУдаления = ЛОЖЬ
			|	И УБ_РолиИсполнителей.ИспользуетсяСОбъектамиАдресации = ИСТИНА
			| СГРУППИРОВАТЬ ПО
			|	УБ_РолиИсполнителей.Ссылка,
			|	УБ_ИсполнителиЗадач.РольИсполнителя, 
			|	УБ_РолиИсполнителей.ВнешняяРоль,
			|	УБ_РолиИсполнителей.Наименование,
			|	УБ_РолиИсполнителей.ТипыОсновногоОбъектаАдресации";
	Иначе
		ТекстЗапроса = 
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			|	УБ_РолиИсполнителей.Ссылка КАК РольСсылка,
			...
			|			КОГДА УБ_ИсполнителиЗадач.РольИсполнителя <> ЗНАЧЕНИЕ(Справочник.УБ_РолиИсполнителей.ПустаяСсылка) 
			|				И УБ_ИсполнителиЗадач.РольИсполнителя ЕСТЬ НЕ NULL 
			|				И (УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации ЕСТЬ NULL 
			|					ИЛИ УБ_ИсполнителиЗадач.ОсновнойОбъектАдресации = Неопределено)
			|				ТОГДА 1
			|			ИНАЧЕ 0
			|		КОНЕЦ) КАК Исполнители
			...
			|ГДЕ
			|	УБ_РолиИсполнителей.ПометкаУдаления = ЛОЖЬ
			|	И УБ_РолиИсполнителей.ИспользуетсяБезОбъектовАдресации = ИСТИНА
			| СГРУППИРОВАТЬ ПО
			|	УБ_РолиИсполнителей.Ссылка,
			|	УБ_ИсполнителиЗадач.РольИсполнителя, 
			|	УБ_РолиИсполнителей.ВнешняяРоль,
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Мультиязычность") Тогда
		...
		Если ЗначениеЗаполнено(СуффиксТекущегоЯзыка) Тогда
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "УБ_РолиИсполнителей.Наименование", 
				"УБ_РолиИсполнителей.Наименование" + СуффиксТекущегоЯзыка);
		КонецЕсли;
...
Функция ВыбратьИсполнителейРоли(РольСсылка, ОсновнойОбъектАдресации = Неопределено,
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ
	   |	УБ_ИсполнителиЗадач.Исполнитель
	   |ИЗ
	   |	РегистрСведений.УБ_ИсполнителиЗадач КАК УБ_ИсполнителиЗадач
	   |ГДЕ
	   |	УБ_ИсполнителиЗадач.РольИсполнителя = &РольИсполнителя";
...
// Параметры:
//   БизнесПроцессОбъект - ОпределяемыйТип.БизнесПроцессОбъект
//   СтароеСостояние - ПеречислениеСсылка.УБ_СостоянияБизнесПроцессов
// 
...
Функция ГруппыИсполнителейЗадач(НовыеИсполнителиЗадач) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(УБ_ГруппыИсполнителейЗадач.Ссылка, ЗНАЧЕНИЕ(Справочник.УБ_ГруппыИсполнителейЗадач.ПустаяСсылка)) КАК Ссылка,
	|	НовыеЗаписи.РольИсполнителя КАК РольИсполнителя,
	|	НовыеЗаписи.ОсновнойОбъектАдресации КАК ОсновнойОбъектАдресации,
	|	НовыеЗаписи.ДополнительныйОбъектАдресации КАК ДополнительныйОбъектАдресации
	|ИЗ
	|	НовыеЗаписи КАК НовыеЗаписи
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УБ_ГруппыИсполнителейЗадач КАК УБ_ГруппыИсполнителейЗадач
	|		ПО НовыеЗаписи.РольИсполнителя = УБ_ГруппыИсполнителейЗадач.РольИсполнителя
	|			И НовыеЗаписи.ОсновнойОбъектАдресации = УБ_ГруппыИсполнителейЗадач.ОсновнойОбъектАдресации
	|			И НовыеЗаписи.ДополнительныйОбъектАдресации = УБ_ГруппыИсполнителейЗадач.ДополнительныйОбъектАдресации";
...
Функция ЕстьПраваНаОстановкуБизнесПроцесса(БизнесПроцесс)
	...
	ЕстьПрава = Ложь;
	СтандартнаяОбработка = Истина;
	УБ_БизнесПроцессыИЗадачиПереопределяемый.ПриПроверкеПравНаОстановкуБизнесПроцесса(БизнесПроцесс, ЕстьПрава, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат ЕстьПрава;
	КонецЕсли;
...
Функция КоличествоЗадачИсполнителя()
	...
	|ИЗ
	|	Задача.УБ_ЗадачаИсполнителя.ЗадачиПоИсполнителю КАК ЗадачиПоИсполнителю
	|ГДЕ
	|	НЕ ЗадачиПоИсполнителю.ПометкаУдаления
	|	И НЕ ЗадачиПоИсполнителю.Выполнена
	|	И ЗадачиПоИсполнителю.СостояниеБизнесПроцесса = ЗНАЧЕНИЕ(Перечисление.УБ_СостоянияБизнесПроцессов.Активен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ЗаполнитьОтветственногоЗаКонтрольИсполнения() Экспорт
	...
	ВсеОбъектыАдресации = ПланыВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ВсеОбъектыАдресации;
	
	РольОбъект = Справочники.УБ_РолиИсполнителей.ОтветственныйЗаКонтрольИсполнения.ПолучитьОбъект();
...
Процедура ОбновитьИспользованиеРегламентныхЗаданий() Экспорт
	...
	ПараметрыЗадания = Новый Структура("Использование", ПолучитьФункциональнуюОпцию("УБ_ИспользоватьБизнесПроцессыИЗадачи"));
...
Процедура ЗаполнитьНаименованиеПредопределенногоЭлементаВсеОбъектыАдресации() Экспорт
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("ПланВидовХарактеристик.УБ_ОбъектыАдресацииЗадач");
	ЭлементБлокировки.УстановитьЗначение("Ссылка", ПланыВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ВсеОбъектыАдресации.Ссылка);
	...
	Попытка
		...
		ВсеОбъектыАдресации = ПланыВидовХарактеристик.УБ_ОбъектыАдресацииЗадач.ВсеОбъектыАдресации.ПолучитьОбъект();
