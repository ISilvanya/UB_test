...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(АдресИсходнойСхемыКомпоновкиДанных) Тогда
...
Функция СформироватьСтруктуруВозврата()
	...
	СтруктураВозврата = Новый Структура("АдресХранилищаНастройкиКомпоновщика, ИмяТекущегоШаблонаСКД, АдресСхемыКомпоновкиДанных");
	...
	СтруктураВозврата.АдресСхемыКомпоновкиДанных = АдресИсходнойСхемыКомпоновкиДанных;
...
Процедура ОткрытьКонструкторСхемыКомпоновкиДанных()
	...
	#Если ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение Тогда
		...
		Если ЗначениеЗаполнено(Отчет) Тогда
			АдресСхемыОтчета = ПолучитьАдресСхемыОтчета();
		КонецЕсли;
		//АдресСхемы = ПолучитьИзВременногоХранилища(АдресСхемыОтчета);
		//Схема = АдресСхемы.Схема;
		// Создается копия редактируемой схемы
		Если ЗначениеЗаполнено(АдресСхемыОтчета) Тогда	
			СхемаКомпоновкиДанных = СериализаторXDTO.ПрочитатьXDTO(СериализаторXDTO.ЗаписатьXDTO(ПолучитьИзВременногоХранилища(АдресСхемыОтчета)));
		ИначеЕсли ЗначениеЗаполнено(АдресРедактируемойСхемыКомпоновкиДанных) тогда	
...
&НаСервере
Функция ПолучитьАдресСхемыОтчета()
	
	Попытка
		Имя = Отчет.Отчет.ИмяОбъекта;
	Исключение
		Имя = Отчет.Отчет.Имя;
	КонецПопытки;         
	//Отчет
	//Схема = Метаданные.НайтиПоПолномуИмени("Отчет." + Имя).Макеты.ОсновнаяСхемаКомпоновкиДанных;
	ИмяМакета = Метаданные.НайтиПоПолномуИмени("Отчет." + Имя).ОсновнаяСхемаКомпоновкиДанных.Имя;
	//Схема = Отчеты.УБ_АнализЭффективностиСотрудников.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных"); 
	Схема = Отчеты[Имя].ПолучитьМакет(ИмяМакета);
	
	//Структура = Новый Структура;
	//Структура.Вставить("Схема",Схема);
	ПоместитьВоВременноеХранилище(Схема, АдресРедактируемойСхемыКомпоновкиДанных);
	
	Возврат АдресРедактируемойСхемыКомпоновкиДанных;
