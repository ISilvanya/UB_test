...
Процедура ВсеНазначенияВыполнить(Команда)
	...
	ОткрытьФорму("РегистрСведений.УБ_ИсполнителиЗадач.ФормаСписка", ПараметрыФормы, ЭтотОбъект);
...
Процедура НазначитьИсполнителей(Команда)
	...
	ОткрытьФорму("РегистрСведений.УБ_ИсполнителиЗадач.Форма.ИсполнителиРолиСОбъектомАдресации", 
...
Процедура СписокРолей(Команда)
	ОткрытьФорму("Справочник.УБ_РолиИсполнителей.ФормаСписка",,ЭтотОбъект);
КонецПроцедуры
...
Процедура ОбновитьДанныеЭлементов()
	...
	ВыборкаЗапроса = УБ_БизнесПроцессыИЗадачиСервер.ВыбратьРолиСКоличествомИсполнителей(ОсновнойОбъектАдресации);
	...
	Для каждого СтрокаСписка Из СписокОбъект Цикл
		...
		ИначеЕсли СтрокаСписка.Исполнители = 1 Тогда
			СтрокаСписка.ИсполнителиСтрока = Строка(УБ_БизнесПроцессыИЗадачиСервер.ВыбратьИсполнителя(ОсновнойОбъектАдресации, СтрокаСписка.РольСсылка));
